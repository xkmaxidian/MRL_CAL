...
def inference(loader, model, device, view, data_size):
    ...
    for step, (xs, y, _) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            hs, qs, xrs, zs, xfakes, d, d_= model.forward(xs)
            q = sum(qs) / view
        for v in range(view):
            hs[v] = hs[v].detach()
            zs[v] = zs[v].detach()
            xfakes[v] = xfakes[v].detach()
            ...
            Xfakes[v].extend(xfakes[v].cpu().detach().numpy())
        q = q.detach()
    ...
    dataNew = 'D://MATLAB//bin//CVPR//modelss.mat'
    scio.savemat(dataNew, {'H1': Hs[0],'H2': Hs[1], 'Z1':Zs[0], 'Z2':Zs[1],'x_f1': Xfakes[0],'x_f2': Xfakes[1],'y': y.numpy()})

    return total_pred, pred_vectors, Hs, labels_vector, Zs, Xfakes
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    total_pred, pred_vectors, high_level_vectors, labels_vector, low_level_vectors, Xfakes = inference(test_loader, model, device, view, data_size)
    if eval_h:
